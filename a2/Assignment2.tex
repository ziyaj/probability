...
\]
\[
Pr(Y = 6) = Pr(Y=6\ |X_1)\cdot Pr(X_1) + Pr(Y=6\ | X_2)\cdot Pr(X_2) = 0.1372
\]
The question is asking about \( Pr(X_1 \ | Y = 6) \). According to \textit{Bayes' Theorem}:
\[
Pr(X_1 \ | Y = 6) = \frac{Pr(Y = 6 \  | X_1) Pr(X_1)}{Pr(Y = 6)} = \frac{0.15625 \times 0.7}{0.1372} = 0.7974
\]


  \item Let Y denote the number of trials until the first head is obtained. \( Y \sim Geom(p) \).\\
It takes at most 4 tosses to obtain the first head means
\[
Pr(Y \leq 4) = Pr(Y = 1) + Pr(Y = 2) + Pr(Y = 3) + Pr(Y = 4)
\]
Let \( X_1 \) denote the event that Peter picks the first type of coin. \(Pr(X_1) = 0.7, \  p = 0.5.\) \\
%Let \( X_2 \) denote the event that Peter picks the second type of coin. \( Pr(X_2) = 0.3, \ p = 0.7.\) \\
\[
Pr(Y \leq 4\ | X_1) = (1-0.5)^{1-1} \ 0.5 + (1-0.5)^{2-1}\ 0.5 + (1-0.5)^{3-1} \ 0.5 + (1 - 0.5)^{4-1} \ 0.5 = \frac{15}{16} = 0.9375
\]
%\[
...
\[
Pr((Y \leq 4) \cap X_1) = Pr(Y \leq 4\ | \ X_1) \cdot Pr(X_1) = 0.9375 \times 0.7 = 0.65625
\]
\end{enumerate}
...
\]
\[
= \frac{{12 \choose 5} {8 \choose 5} }{{20 \choose 10}} + \frac{{12 \choose 6} {8 \choose 4}}{{20 \choose 10}} + \frac{{12 \choose 7} {8 \choose 3}}{{20 \choose 10}} = 0.8301977
\]
